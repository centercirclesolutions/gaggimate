...
bool BleClientTransport::connectToServer() {
    ...
    // Emit (and reset the endpoint session) BEFORE subscribing, else the controller's
    // SystemInfo push + our config burst race ahead and get wiped by the late reset -> no-heat on reconnect.
    emitConnection(true);
