...
void Controller::setupWifi() {
    if (settings.getWifiSsid() != "" && settings.getWifiPassword() != "") {
        ...
        // Before begin() so boot-connect events fire through these handlers too.
        WiFi.onEvent(
        ...
        // setMinSecurity() is a scan filter, not an auth ceiling -- log what was negotiated.
        WiFi.onEvent(
        ...
        // disconnectReasonName() returns NULL for vendor codes (UniFi 81/168).
        WiFi.onEvent(
            [this](WiFiEvent_t, WiFiEventInfo_t info) {
    ...
    // AP path has no STA_GOT_IP; STA path dispatches from the handler.
    if (isApConnection) {
...
void Controller::loop() {
    // Handlers only flag (small-stack event task); dispatch here, disconnect
    // before connect so a flap is ordered correctly.
