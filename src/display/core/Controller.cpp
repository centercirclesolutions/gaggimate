...
void Controller::setupWifi() {
    if (settings.getWifiSsid() != "" && settings.getWifiPassword() != "") {
        ...
        // Register handlers BEFORE begin() so boot-connect events fire through
        // them too. Handlers run on the WiFi event task -- log and flag only.
        WiFi.onEvent(
        ...
        // disconnectReasonName() returns NULL for vendor codes (UniFi 168).
        WiFi.onEvent(
            [this](WiFiEvent_t, WiFiEventInfo_t info) {
